...
//            effectInContext.draw(self.cgImage, in: imageRect)
            ...
            if hasBlur {
                ...
                let constantMultiplier = 3.0 * CGFloat(sqrt(2 * M_PI)) / 4
                var radius = UInt32(floor(inputRadius * constantMultiplier + 0.5))
                if radius % 2 != 1 {
        ...
        if hasBlur {
            ...
            if let image = maskImage {
                ...
                if let effectCGImage = effectImage.cgImage!.masking(image.cgImage!) {
